  ...
  describe('parameter validation', () => {
    test('should require foreground parameter', async () => {
      ...
      if (!result.success) {
        ...
        expect((result as any).error.message).toContain('Foreground color parameter is required');
      }
    });
    ...
    test('should require background parameter', async () => {
      ...
      if (!result.success) {
        expect((result as any).error.code).toBe('MISSING_PARAMETER');
        expect((result as any).error.message).toContain('Background color parameter is required');
      }
    });
    ...
    test('should validate foreground color format', async () => {
      ...
      if (!result.success) {
        ...
        expect((result as any).error.message).toContain('Invalid foreground color format');
      }
    });
    ...
    test('should validate background color format', async () => {
      ...
      if (!result.success) {
        ...
        expect((result as any).error.message).toContain('Invalid background color format');
      }
    });
  });
  ...
  describe('contrast calculations', () => {
    ...
    test('should handle different text sizes', async () => {
      ...
      if (normalResult.success && largeResult.success) {
        ...
        expect((normalResult.data as any).contrast_ratio).toBe((largeResult.data as any).contrast_ratio);
        ...
        if ((normalResult.data as any).contrast_ratio >= 3.0 && (normalResult.data as any).contrast_ratio < 4.5) {
  ...
  describe('recommendations', () => {
    test('should provide recommendations for poor contrast', async () => {
      ...
      if (result.success) {
        ...
        expect((result.data as any).recommendations.some((rec: any) => 
    ...
    test('should suggest specific improvements', async () => {
      ...
      if (result.success) {
        expect((result.data as any).compliance.passes).toBe(false);
        expect((result.data as any).recommendations.some((rec: any) => 
          rec.includes('darker') || rec.includes('lighter')
        )).toBe(true);
      }
    });
  });
  ...
  describe('alternative combinations', () => {
    test('should provide alternatives for poor contrast', async () => {
      ...
      if (result.success) {
        ...
        expect(bestFgAlt.contrast_ratio).toBeGreaterThan((result.data as any).contrast_ratio);
        expect(bestBgAlt.contrast_ratio).toBeGreaterThan((result.data as any).contrast_ratio);
      }
    });
    ...
    test('should sort alternatives by contrast ratio', async () => {
      ...
      if (result.success && (result.data as any).alternative_combinations) {
        const fgAlts = (result.data as any).alternative_combinations.foreground_adjustments;
        const bgAlts = (result.data as any).alternative_combinations.background_adjustments;
        
        // Should be sorted by contrast ratio (descending)
        for (let i = 1; i < fgAlts.length; i++) {
          expect(fgAlts[i-1].contrast_ratio).toBeGreaterThanOrEqual(fgAlts[i].contrast_ratio);
        }
        
        for (let i = 1; i < bgAlts.length; i++) {
          expect(bgAlts[i-1].contrast_ratio).toBeGreaterThanOrEqual(bgAlts[i].contrast_ratio);
        }
      }
    });
  });
  ...
  describe('performance requirements', () => {
    ...
    test('should handle multiple checks efficiently', async () => {
      ...
      const results = await Promise.all(
        combinations.map(combo => checkContrast({
          foreground: combo.fg,
          background: combo.bg
        }))
      );
  ...
  describe('metadata and response format', () => {
    ...
    test('should provide accessibility notes', async () => {
      ...
      if (goodResult.success) {
        expect(goodResult.metadata.accessibility_notes!.some(note => 
          note.includes('AAA') || note.includes('Excellent')
        )).toBe(true);
      }
      
      if (poorResult.success) {
        expect(poorResult.metadata.accessibility_notes!.some(note => 
          note.includes('does not meet') || note.includes('WCAG')
        )).toBe(true);
      }
    });
