...
describe('Comprehensive Color Format Conversion Tests', () => {
  ...
  describe('All Format Combinations', () => {
    test('should convert between all supported input and output formats', async () => {
      const inputFormats = ['hex', 'rgb', 'hsl', 'hsv', 'hwb', 'cmyk', 'lab', 'xyz', 'lch', 'oklab', 'oklch', 'named'];
      
      for (const inputFormat of inputFormats) {
        const inputColor = testColors.red[inputFormat as keyof typeof testColors.red];
  ...
  describe('Advanced Color Space Conversions', () => {
    ...
    test('should handle XYZ color space conversions', async () => {
      ...
      for (const input of xyzInputs) {
        ...
        expect(result.success).toBe(true);
        expect((result.data as any).converted).toMatch(/^rgb\(\d+, \d+, \d+\)$/);
    ...
    test('should handle LCH color space conversions', async () => {
      ...
      for (const input of lchInputs) {
        ...
        expect(result.success).toBe(true);
        expect((result.data as any).converted).toMatch(/^hsl\(\d+\.\d+, \d+\.\d+%, \d+\.\d+%\)$/);
  ...
  describe('Framework-Specific Format Testing', () => {
    test('should generate accurate Swift UIColor formats', async () => {
      const testCases = [
        { input: '#FF0000', expected: 'UIColor(red: 1.000, green: 0.000, blue: 0.000, alpha: 1.000)' },
        { input: '#00FF00', expected: 'UIColor(red: 0.000, green: 1.000, blue: 0.000, alpha: 1.000)' },
        { input: '#0000FF', expected: 'UIColor(red: 0.000, green: 0.000, blue: 1.000, alpha: 1.000)' },
        { input: '#808080', expected: 'UIColor(red: 0.502, green: 0.502, blue: 0.502, alpha: 1.000)' }
      ];
    ...
    test('should generate accurate Android Color formats', async () => {
      const testCases = [
        ...
        { input: 'rgba(255, 0, 0, 0.5)', expectedPattern: /Color\.parseColor\("#[0-9A-F]{8}"\)/ }
      ];
      ...
      for (const testCase of testCases) {
        ...
        if ('expected' in testCase) {
          ...
          expect((result.data as any).converted).toMatch(testCase.expectedPattern);
        }
      }
    });
    ...
    test('should generate accurate Flutter Color formats', async () => {
      const testCases = [
        ...
        { input: 'rgba(255, 0, 0, 0.5)', expectedPattern: /Color\(0x[0-9A-F]{8}\)/ }
      ];
  ...
  describe('CSS and SCSS Variable Generation', () => {
    test('should generate CSS variables with custom names', async () => {
      ...
      expect((result.data as any).css_variable).toBe('--primary-color: #ff0000;');
    });
    ...
    test('should generate SCSS variables with custom names', async () => {
      ...
      expect(result.success).toBe(true);
      expect((result.data as any).converted).toBe('$color: #ff0000;');
      expect((result.data as any).scss_variable).toBe('$accent_color: #ff0000;');
    });
  });
  ...
  describe('Named Color Conversions', () => {
    test('should convert named colors to all formats', async () => {
      const namedColors = [
        ...
        'black', 'white', 'gray', 'orange', 'purple', 'pink',
  ...
  describe('High Precision Conversions', () => {
    test('should handle precision parameter for all decimal formats', async () => {
      const precisionTests = [
        ...
        { format: 'hsl', precision: 1, pattern: /hsl\(\d+\.\d, \d+\.\d%, \d+\.\d%\)/ },
        { format: 'hsl', precision: 3, pattern: /hsl\(\d+\.\d{3}, \d+\.\d{3}%, \d+\.\d{3}%\)/ },
        { format: 'lab', precision: 2, pattern: /lab\(\d+\.\d{2}, -?\d+\.\d{2}, -?\d+\.\d{2}\)/ },
        { format: 'xyz', precision: 4, pattern: /xyz\(\d+\.\d{4}, \d+\.\d{4}, \d+\.\d{4}\)/ },
        { format: 'lch', precision: 1, pattern: /lch\(\d+\.\d, \d+\.\d, \d+\.\d\)/ },
        { format: 'oklab', precision: 5, pattern: /oklab\(\d+\.\d{5}, -?\d+\.\d{5}, -?\d+\.\d{5}\)/ },
        { format: 'oklch', precision: 3, pattern: /oklch\(\d+\.\d{3}, \d+\.\d{3}, \d+\.\d{3}\)/ }
      ];
    ...
    test('should maintain accuracy in high-precision round-trip conversions', async () => {
      ...
      for (const originalColor of originalColors) {
        ...
        const converted = parseInt((backToHex.data as any).converted.slice(1), 16);
  ...
  describe('Edge Cases and Error Handling', () => {
    test('should handle extreme color values correctly', async () => {
      const extremeCases = [
        ...
        { input: 'lab(50, -128, -128)', description: 'LAB negative extreme chroma' },
  ...
  describe('Mathematical Accuracy Validation', () => {
    test('should maintain color relationships in conversions', async () => {
      ...
      const redHue = parseFloat((redResult.data as any).converted.match(/hsl\(([\d.]+)/)[1]);
      const cyanHue = parseFloat((cyanResult.data as any).converted.match(/hsl\(([\d.]+)/)[1]);
    ...
    test('should preserve grayscale properties', async () => {
      const grayscaleColors = ['#000000', '#404040', '#808080', '#C0C0C0', '#FFFFFF'];
      ...
      for (const color of grayscaleColors) {
        ...
        const saturation = parseFloat((hslResult.data as any).converted.match(/hsl\([\d.]+, ([\d.]+)%/)[1]);
    ...
    test('should handle color gamut boundaries correctly', async () => {
      ...
      for (const color of gamutEdgeCases) {
        ...
        const rgbMatch = (backToRgb.data as any).converted.match(/rgb\((\d+), (\d+), (\d+)\)/);
