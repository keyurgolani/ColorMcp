  ...
  describe('parameter validation', () => {
    test('should require color parameter', async () => {
      ...
      if (!result.success) {
        ...
        expect((result as any).error.message).toContain('Color parameter is required');
      }
    });
    ...
    test('should validate color format', async () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect((result as any).error.code).toBe('INVALID_COLOR_FORMAT');
    ...
    test('should validate compare_color format if provided', async () => {
      ...
      expect(result.success).toBe(false);
      if (!result.success) {
        expect((result as any).error.code).toBe('INVALID_COMPARE_COLOR');
      }
    });
  });
  ...
  describe('successful analysis', () => {
    test('should analyze a basic color', async () => {
      ...
      if (result.success) {
        ...
        expect((result.data as any).analysis.brightness.perceived_brightness).toBeGreaterThan(0);
        expect((result.data as any).analysis.brightness.brightness_category).toBeDefined();
        expect(typeof (result.data as any).analysis.brightness.is_light).toBe('boolean');
        
        // Check temperature analysis
        expect(['warm', 'cool', 'neutral']).toContain((result.data as any).analysis.temperature.temperature);
        expect((result.data as any).analysis.temperature.warmth_score).toBeGreaterThanOrEqual(-1);
        expect((result.data as any).analysis.temperature.warmth_score).toBeLessThanOrEqual(1);
        
        // Check summary
        expect((result.data as any).summary.overall_score).toBeGreaterThanOrEqual(0);
        expect((result.data as any).summary.overall_score).toBeLessThanOrEqual(100);
        expect(Array.isArray((result.data as any).summary.primary_issues)).toBe(true);
        expect(Array.isArray((result.data as any).summary.strengths)).toBe(true);
      }
    });
    ...
    test('should analyze color with comparison', async () => {
      ...
      if (result.success) {
        ...
        expect((result.data as any).analysis.distance?.cie2000).toBeGreaterThan(5);
        expect((result.data as any).analysis.distance?.perceptual_difference).toBe('very_different');
      }
    });
    ...
    test('should provide meaningful recommendations', async () => {
      ...
      if (poorColor.success) {
        ...
        expect(Array.isArray((poorColor.data as any).analysis.accessibility.recommendations)).toBe(true);
      }
    });
  });
  ...
  describe('metadata and response format', () => {
    ...
    test('should provide accessibility notes for problematic colors', async () => {
      ...
      if (result.success) {
        ...
        if (result.metadata.accessibility_notes!.length > 0) {
          expect(result.metadata.accessibility_notes!.some(note => 
            note.includes('WCAG') || note.includes('contrast') || note.includes('brightness') || note.includes('temperature')
          )).toBe(true);
        }
      }
    });
    ...
    test('should limit recommendations to reasonable number', async () => {
      ...
      if (result.success) {
        ...
        expect((result.data as any).summary.primary_issues.length).toBeLessThanOrEqual(3);
        expect((result.data as any).summary.strengths.length).toBeLessThanOrEqual(3);
      }
    });
  });
  ...
  describe('summary generation', () => {
    test('should generate meaningful summary for good colors', async () => {
      ...
      if (result.success) {
        ...
        expect(summary.strengths.some((strength: any) => 
    ...
    test('should identify issues with problematic colors', async () => {
      ...
      if (result.success) {
        ...
        if (summary.primary_issues.length > 0) {
          expect(summary.primary_issues.some((issue: any) => 
            issue.includes('contrast') || issue.includes('text') || issue.includes('accessibility')
          )).toBe(true);
        }
      }
    });
    ...
    test('should provide balanced assessment', async () => {
      ...
      if (result.success) {
        ...
        const totalFeedback = summary.strengths.length + summary.primary_issues.length;
