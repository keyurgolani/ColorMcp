...
describe('Core Color Conversion Performance', () => {
  ...
  describe('UnifiedColor Performance', () => {
    test('should create UnifiedColor instances under 1ms each', () => {
      ...
      console.log(`UnifiedColor creation: ${totalTime}ms for 1000 instances (${avgTime.toFixed(3)}ms per instance)`);
    });

    test('should convert formats under 0.1ms each', () => {
      const color = new UnifiedColor('#FF0000');
      const formats = ['hex', 'rgb', 'hsl', 'hsv', 'hwb', 'cmyk', 'lab', 'xyz', 'lch'];
      ...
      console.log(`Format conversion: ${totalTime}ms for 1000 conversions (${avgTime.toFixed(3)}ms per conversion)`);
    });
    ...
    test('should handle all format getters efficiently', () => {
      ...
      expect(avgTime).toBeLessThan(0.1);
      console.log(`Format getters: ${totalTime}ms for 9000 getter calls (${avgTime.toFixed(4)}ms per getter)`);
    });
  });
  ...
  describe('ColorParser Performance', () => {
    test('should parse colors under 0.2ms each', () => {
      ...
      expect(avgTime).toBeLessThan(0.5); // Very tolerant for CI environments
      console.log(`Color parsing: ${totalTime}ms for 1000 parses (${avgTime.toFixed(3)}ms per parse)`);
    });
    ...
    test('should handle complex format parsing efficiently', () => {
      ...
      expect(avgTime).toBeLessThan(0.5); // Very tolerant for CI environments
      console.log(`Complex parsing: ${totalTime}ms for 1000 complex parses (${avgTime.toFixed(3)}ms per parse)`);
    });
  });
  ...
  describe('Round-trip Conversion Performance', () => {
    test('should handle round-trip conversions efficiently', () => {
      ...
      expect(avgTime).toBeLessThan(1);
      console.log(`Round-trip conversions: ${totalTime}ms for 800 operations (${avgTime.toFixed(3)}ms per operation)`);
    });
  });
  ...
  describe('Memory Efficiency', () => {
    test('should not leak memory during intensive operations', () => {
      ...
      console.log(`Memory efficiency: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB increase for 1000 color instances`);
    });
  });
  ...
  describe('Concurrent Operations', () => {
    test('should handle concurrent color operations', async () => {
      ...
      console.log(`Concurrent operations: ${totalTime}ms for 100 concurrent color operations`);
    });
  });
  ...
  describe('Format-Specific Performance', () => {
    test('should handle HEX conversions efficiently', () => {
      ...
      console.log(`HEX format: ${totalTime}ms for 10000 conversions (${avgTime.toFixed(4)}ms per conversion)`);
    });
    ...
    test('should handle complex format conversions efficiently', () => {
      ...
      console.log(`Complex formats: ${totalTime}ms for 1000 conversions (${avgTime.toFixed(3)}ms per conversion)`);
    });
    ...
    test('should handle framework formats efficiently', () => {
      ...
      console.log(`Framework formats: ${totalTime}ms for 1000 conversions (${avgTime.toFixed(3)}ms per conversion)`);
    });
  });
});