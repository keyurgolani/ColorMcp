...
describe('Palette Generation Performance Benchmarks', () => {
  ...
  describe('PaletteGenerator Performance', () => {
    test('should generate single harmony palette within 500ms', () => {
      ...
      expect(palette.metadata.generationTime).toBeLessThan(performanceThresholds.singlePalette);
    ...
    test('should maintain performance across different harmony types', () => {
      ...
      results.forEach(result => {
        expect(result.executionTime).toBeLessThan(performanceThresholds.singlePalette);
        expect(result.generationTime).toBeLessThan(performanceThresholds.singlePalette);
      });

      // Log performance results for analysis
      console.log('Harmony Type Performance Results:');
      results.forEach(result => {
        console.log(`${result.harmonyType}: ${result.executionTime.toFixed(2)}ms (internal: ${result.generationTime}ms)`);
      });
    });
    ...
    test('should scale linearly with palette size', () => {
      ...
      results.forEach(result => {
        console.log(`Count ${result.count}: ${result.executionTime.toFixed(2)}ms (internal: ${result.generationTime}ms)`);
      });
    });
  });
  ...
  describe('MCP Tool Performance', () => {
    test('should handle MCP tool calls within performance requirements', async () => {
      ...
      expect(executionTime).toBeLessThan(performanceThresholds.singlePalette);
      expect(result.metadata.execution_time).toBeLessThan(performanceThresholds.singlePalette);
    });
    ...
    test('should handle concurrent MCP tool calls efficiently', async () => {
      ...
      console.log(`Concurrent execution of 5 requests: ${totalTime.toFixed(2)}ms`);
    });
  ...
  describe('Memory Usage', () => {
    test('should not cause memory leaks during repeated generation', () => {
      ...
      console.log(`Memory increase after 100 palettes: ${(memoryIncrease / 1024 / 1024).toFixed(2)}MB`);
    });
    ...
    test('should handle large palettes without excessive memory usage', () => {
      ...
      console.log(`Memory for single large palette: ${(memoryIncrease / 1024).toFixed(2)}KB`);
    });
  });
  ...
  describe('Stress Testing', () => {
    test('should handle rapid successive generations', () => {
      ...
      for (let i = 0; i < iterations; i++) {
        const palette = PaletteGenerator.generateHarmonyPalette({
          ...
          harmonyType: ['complementary', 'triadic', 'analogous'][i % 3] as HarmonyType,
          count: 3 + (i % 8), // 3-10
      ...
      console.log(`Stress test: ${iterations} palettes in ${totalTime.toFixed(2)}ms (avg: ${avgTime.toFixed(2)}ms)`);
    });
    ...
    test('should maintain accuracy under stress', () => {
      ...
      console.log(`Stress test quality - Harmony: ${avgHarmonyScore.toFixed(1)}, Diversity: ${avgDiversityScore.toFixed(1)}`);
