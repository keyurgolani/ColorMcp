...
export class PaletteGenerator {
  ...
  static generateHarmonyPalette(options: HarmonyGenerationOptions): Palette {
    ...
    const colors = this.generateHarmonyColors(baseColor, options.harmonyType, count, variation);
    
    // Calculate relationships
    const relationships = this.calculateRelationships(colors, options.harmonyType);
    
    // Calculate metrics
    const diversity = this.calculateDiversity(colors);
    const harmonyScore = this.calculateHarmonyScore(colors, options.harmonyType);
  ...
  private static validateOptions(options: HarmonyGenerationOptions): void {
    ...
    if (!validHarmonyTypes.includes(options.harmonyType)) {
      throw new Error(`Invalid harmony type: ${options.harmonyType}. Valid types: ${validHarmonyTypes.join(', ')}`);
    }

    if (options.count !== undefined && (options.count < 3 || options.count > 10)) {
      throw new Error('Count must be between 3 and 10');
    }

    if (options.variation !== undefined && (options.variation < 0 || options.variation > 100)) {
  ...
  private static generateMonochromatic(
    ...
    for (let i = 1; i < count; i++) {
      ...
      const newColor = UnifiedColor.fromHsl(baseHsl.h, newSaturation, newLightness);
  ...
  private static generateAnalogous(
    ...
    for (let i = 1; i < count; i++) {
      ...
      let newHue = (baseHsl.h + angle + 360) % 360;
      ...
      let newSaturation = Math.max(0, Math.min(100, baseHsl.s + satVariation));
      let newLightness = Math.max(10, Math.min(90, baseHsl.l + lightVariation));
      
      const newColor = UnifiedColor.fromHsl(newHue, newSaturation, newLightness);
      colors.push(newColor);
    }
  ...
  private static generateComplementary(
    ...
    for (let i = 2; i < count; i++) {
      ...
      let newHue = (sourceHue + hueVariation + 360) % 360;
      let newSaturation = Math.max(0, Math.min(100, baseHsl.s + satVariation));
      let newLightness = Math.max(10, Math.min(90, baseHsl.l + lightVariation));
  ...
  private static calculateHarmonyScore(
    ...
    for (let i = 1; i < colors.length; i++) {
      ...
      if (deviation > 15) { // Allow 15-degree tolerance
