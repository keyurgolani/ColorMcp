...
/**
 * Generate PNG from color comparison
 */
...
/**
 * Create color comparison PNG tool handler
 */
async function createColorComparisonPng(
  ...
  try {
    ...
    // Generate PNG
    const pngBuffer = await generateColorComparisonPng(validatedParams);
    const base64Png = pngBuffer.toString('base64');

    // Calculate final dimensions
    const [width, height] = calculateComparisonDimensions(
      validatedParams.color_sets,
      validatedParams.comparison_type || 'side_by_side',
      validatedParams.format_for || 'web',
