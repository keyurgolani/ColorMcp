...
export async function analyzeColor(
  ...
  try {
    ...
    const colorValidation = validateColorInput(params.color);
    ...
    // Parse colors
    const color = new UnifiedColor(params.color);
    let compareColor: UnifiedColor | undefined;

    if (params.compare_color) {
      const compareValidation = validateColorInput(params.compare_color);
      if (!compareValidation.isValid) {
        return createErrorResponse(
          'analyze_color',
          'INVALID_COMPARE_COLOR',
          `Invalid compare color format: ${params.compare_color}`,
          Date.now() - startTime,
          {
  ...
  parameters: {
    ...
    properties: {
      ...
      analysis_types: {
        ...
        enum: ['brightness', 'temperature', 'contrast', 'accessibility', 'all'],
        default: ['all'],
      },
      compare_color: {
        type: 'string',
