 ...
 */
export async function optimizeForAccessibility(params: OptimizeForAccessibilityParams): Promise<ToolResponse | ErrorResponse> {
  const startTime = Date.now();

  try {
    // Validate required parameters
    if (!params.palette || !Array.isArray(params.palette) || params.palette.length === 0) {
    ...
    if (!params.use_cases || !Array.isArray(params.use_cases) || params.use_cases.length === 0) {
      return createErrorResponse(
        'optimize_for_accessibility',
        'MISSING_USE_CASES',
        ...
        Date.now() - startTime,
        {
          ...
          suggestions: ['Specify use cases: text, background, accent, interactive']
        }
      );
    }
    ...
    const invalidUseCases = params.use_cases.filter(uc => !validUseCases.includes(uc));
    if (invalidUseCases.length > 0) {
      return createErrorResponse(
        ...
        Date.now() - startTime,
        {
          ...
          suggestions: ['Use only valid use cases: text, background, accent, interactive']
        }
      );
    }
    ...
    for (let i = 0; i < validatedColors.length; i++) {
      ...
      for (const useCase of params.use_cases) {
        ...
        totalContrastImprovement += optimizationResult.contrast_improvement.improvement_percentage;

        // Count compliance rates
        if (targetStandard === 'WCAG_AA') {
          if (optimizationResult.accessibility_compliance.wcag_aa_before) complianceBefore++;
          if (optimizationResult.accessibility_compliance.wcag_aa_after) complianceAfter++;
        } else {
          if (optimizationResult.accessibility_compliance.wcag_aaa_before) complianceBefore++;
          if (optimizationResult.accessibility_compliance.wcag_aaa_after) complianceAfter++;
        }
      }
    }
    ...
    const averageContrastImprovement = totalResults > 0 ? totalContrastImprovement / totalResults : 0;
    const complianceRateBefore = totalResults > 0 ? (complianceBefore / totalResults) * 100 : 0;
    const complianceRateAfter = totalResults > 0 ? (complianceAfter / totalResults) * 100 : 0;

    // Generate accessibility notes and recommendations
    const accessibilityNotes = generateAccessibilityNotes(optimizationResults, targetStandard);
    ...
    const responseData: OptimizeForAccessibilityResponse = {
      ...
      summary: {
        ...
        average_contrast_improvement: Math.round(averageContrastImprovement * 100) / 100,
...
async function optimizeColorForUseCase(
  ...
  if (shouldPreserve) {
    ...
    switch (useCase) {
      case 'text':
        optimizedColor = optimizeForText(originalColor, targetStandard, preserveHue);
        break;
      case 'background':
        optimizedColor = optimizeForBackground(originalColor, targetStandard, preserveHue);
        break;
      case 'accent':
        optimizedColor = optimizeForAccent(originalColor, targetStandard, preserveHue);
        break;
      case 'interactive':
        optimizedColor = optimizeForInteractive(originalColor, targetStandard, preserveHue);
        break;
    }
    ...
    if (optimizationApplied) {
      ...
      if (Math.abs(originalHsl.l - optimizedHsl.l) > 5) {
        changesMade.push(`Lightness adjusted from ${Math.round(originalHsl.l)}% to ${Math.round(optimizedHsl.l)}%`);
      }
      
      if (Math.abs(originalHsl.s - optimizedHsl.s) > 5) {
        changesMade.push(`Saturation adjusted from ${Math.round(originalHsl.s)}% to ${Math.round(optimizedHsl.s)}%`);
      }
      
      if (!preserveHue && Math.abs(originalHsl.h - optimizedHsl.h) > 5) {
        changesMade.push(`Hue adjusted from ${Math.round(originalHsl.h)}° to ${Math.round(optimizedHsl.h)}°`);
      }
    }
  }
  ...
  const contrastBefore = ColorAnalyzer.checkContrast(originalColor, whiteBackground).ratio;
  const contrastAfter = ColorAnalyzer.checkContrast(optimizedColor, whiteBackground).ratio;
  const improvementPercentage = contrastBefore > 0 ? ((contrastAfter - contrastBefore) / contrastBefore) * 100 : 0;
 ...
 */
function optimizeForText(color: UnifiedColor, _targetStandard: string, preserveHue: boolean): UnifiedColor {
  const hsl = color.hsl;
  ...
  let optimizedSaturation = Math.min(100, hsl.s + 10);
  
  // Preserve hue if requested
  const optimizedHue = preserveHue ? hsl.h : hsl.h;
  
  return UnifiedColor.fromHsl(optimizedHue, optimizedSaturation, optimizedLightness);
}

/**
 * Optimize color for background use case
 */
function optimizeForBackground(color: UnifiedColor, _targetStandard: string, preserveHue: boolean): UnifiedColor {
  const hsl = color.hsl;
 ...
 */
function optimizeForAccent(color: UnifiedColor, _targetStandard: string, preserveHue: boolean): UnifiedColor {
  const hsl = color.hsl;
 ...
 */
function optimizeForInteractive(color: UnifiedColor, _targetStandard: string, preserveHue: boolean): UnifiedColor {
  const hsl = color.hsl;
  ...
  for (const textResult of textColors) {
    for (const bgResult of backgroundColors) {
      try {
        ...
        const isCompliant = targetStandard === 'WCAG_AAA' ? contrastCheck.wcag_aaa : contrastCheck.wcag_aa;
...
function generateAccessibilityNotes(
  ...
  const complianceImproved = results.filter(r => {
    const beforeCompliant = targetStandard === 'WCAG_AAA' ? 
      r.accessibility_compliance.wcag_aaa_before : 
      r.accessibility_compliance.wcag_aa_before;
    const afterCompliant = targetStandard === 'WCAG_AAA' ? 
  ...
  if (optimizedCount > 0) {
    notes.push(`${optimizedCount} colors were optimized for better accessibility`);
  }

  if (complianceImproved > 0) {
    notes.push(`${complianceImproved} colors now meet ${targetStandard} standards`);
  }
...
function generateOptimizationRecommendations(
  ...
  const stillNonCompliant = results.filter(r => !r.accessibility_compliance.wcag_aa_after).length;
  if (stillNonCompliant > 0) {
    recommendations.push(`${stillNonCompliant} colors still need manual review`);
  }
...
export const optimizeForAccessibilityTool = {
  ...
  description: 'Optimize colors for accessibility compliance while preserving hue when possible',
  parameters: {
    type: 'object',
    properties: {
      palette: {
  ...
  handler: async (params: unknown) => optimizeForAccessibility(params as OptimizeForAccessibilityParams),
};